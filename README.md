...
## Стоковые команды:
- `.addmod` - добавление модуля. Отправляется реплаем на файл с модулем. Пользователь должен убедиться, что все зависимости нового модуля установлены вручную (например, через `pip install <библиотека>`).
